...
class Job(Base):
    ...
    sourcing_enabled: Mapped[bool] = mapped_column(Boolean, server_default="false", nullable=False)
    ...
    criteria: Mapped[JobCriteria | None] = relationship(
        "JobCriteria", back_populates="job", uselist=False, lazy="raise"
    )
    setup_conversation: Mapped[SetupConversation | None] = relationship(
        "SetupConversation", back_populates="job", uselist=False, lazy="raise"
    )
