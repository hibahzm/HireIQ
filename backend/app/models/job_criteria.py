...
class JobCriteria(Base):
    ...
    job_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey("jobs.id", ondelete="CASCADE"), nullable=False, unique=True)
    company_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey("companies.id", ondelete="CASCADE"), nullable=False)
    required_skills: Mapped[list] = mapped_column(JSONB, server_default="'[]'::jsonb", nullable=False)
    optional_skills: Mapped[list] = mapped_column(JSONB, server_default="'[]'::jsonb", nullable=False)
