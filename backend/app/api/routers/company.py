...
async def update_company_overview(
    ...
    company = await CompanyRepository(session).update_overview(current_user.company_id, overview)
    if not company:
        raise HTTPException(status_code=404, detail="Company not found")
...
async def list_company_audit(
    ...
    rows = await AuditLogRepository(session).list_by_company(current_user.company_id, limit=limit)
    return [
        AuditEvent(
            id=str(r["id"]),
