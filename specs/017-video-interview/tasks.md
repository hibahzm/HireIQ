...
- [X] T006 Add `streaming_mode` to the `InterviewSession` model in `backend/app/models/interview_session.py`, add a job-level `streaming_interview` toggle (default off) set during job setup, and seed `streaming_mode` from that job toggle at session creation in the interview session repository/creation path (`backend/app/repositories/interview_session_repository.py`) — this is the concrete source for FR-005's per-job selection; both default off so existing jobs/sessions stay turn-based
