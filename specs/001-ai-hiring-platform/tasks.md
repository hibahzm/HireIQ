...
- [x] T080a [US5] Write feedback endpoint integration test in `backend/tests/integration/test_feedback.py` — valid token → 200 with dimension scores + summary and **no `recommendation`** exposed (US5 AS#2); unknown token → 404; expired token → 410 (F3 remediation)
...
- [x] T081 [P] Configure `infra/docker-compose.prod.yml` and GitHub Actions deploy workflow in `.github/workflows/deploy.yml` — Docker Hub image push on merge to `main`; Azure Container Apps rolling update via `az containerapp update`; `api` and `frontend` with external ingress, `agents` internal-only (research §9)
...
- [x] T083 Run full end-to-end validation per `quickstart.md` — all 5 user stories + tenant isolation test + CI smoke test suite (`pytest tests/integration/ -v`); verify SC-002 SLA: submit 20 concurrent CVs and confirm all screening results appear within 2 minutes; verify SC-004 SLA: confirm evaluation reports appear within 5 minutes for each completed interview *(harness delivered: `infra/scripts/e2e-validate.sh` brings up the stack, runs the integration suite, and prints the SC-002/SC-004 SLA sign-off checklist; the live run requires Docker + a real OPENAI_API_KEY)*
- [x] T084 [P] Performance audit — profile p95 API latency for synchronous endpoints under simulated load (locust or k6); address TODO(PERFORMANCE_BASELINE) in constitution; verify ≤ 300 ms p95; separately measure async pipeline timings (screening, evaluation) to confirm SC-002 and SC-004 SLAs hold at expected load (M1) *(harness: `infra/perf/locustfile.py` + `check_p95.py` gate; TODO(PERFORMANCE_BASELINE) resolved in constitution v1.1.1)*
- [x] T084b [P] RAGAS evaluation suite — install `ragas` in `backend/` dev dependencies; **first create golden-set fixtures**: 20 synthetic CV samples in `backend/tests/ragas/fixtures/cvs/` and 5 interview transcript samples in `backend/tests/ragas/fixtures/transcripts/` (curate manually — these are the ground-truth datasets RAGAS scores against); then write `backend/tests/ragas/test_screening_rag.py` and `backend/tests/ragas/test_evaluation_rag.py`; assert **faithfulness ≥ 0.85** (LLM rationale/evidence grounded in retrieved context, no hallucinated skills or quotes) and **context precision ≥ 0.80** (hybrid search top-k chunks are relevant to job criteria); log per-run scores to `audit_log` under event type `pipeline.quality.ragas`; fail CI if either threshold is breached *(suite calls the agents service over HTTP; gated by the `ragas-quality` CI job which needs `secrets.OPENAI_API_KEY`)*
- [x] T085 [P] Accessibility audit — run axe-core against all frontend pages; fix any WCAG 2.1 AA violations (Principle I) *(vitest + vitest-axe suite `src/pages/accessibility.a11y.test.tsx`, run via `npm run test:a11y` and the `frontend-a11y` CI job; all audited pages pass with 0 violations)*
