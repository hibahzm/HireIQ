...
- [x] T011 Implement async SQLAlchemy engine and `get_db()` dependency in `backend/app/db.py` — `create_async_engine` with asyncpg; `get_db()` runs `SET LOCAL app.current_company_id = :cid` inside every transaction before yielding the session (research §1 pattern)
...
- [x] T028 [US1] Implement FastAPI `Depends()` wiring in `backend/app/api/deps.py` — `get_current_user()` (validates access token, sets `request.state.company_id`), `require_admin()`, `require_recruiter_or_admin()`
